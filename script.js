...
window.onload = function () {
  async function languages() {
    ...
    info = await import('./languageEn.js');
    ...
    async function showWeatherNow(city) {
      try {
        ...
        feelsLike.textContent = isFarengeit
          ? `${info.info.summary.feels} ${
              Math.round(data[0].main.feels_like) + '°'
            }`
          : `${info.info.summary.feels} ${
              Math.round(data[0].main.feels_like * (9 / 5) + 32) + '°'
            }`;
        humidity.textContent = `${info.info.summary.humidity} ${data[0].main.humidity}%`;
        speedWind.textContent = `${
          info.info.summary.wind
        } ${data[0].wind.speed.toFixed()} ${info.info.summary.speed}`;
    ...
    function showTime() {
      ...
      dateNow.textContent = `${info.info.dayOfWeekAbbreviated[dayofWeek]} ${dayNumber} ${info.info.months[month]}`;
      dayofWeek++;
      if (dayofWeek > 6) {
        dayofWeek = 0;
        firstDay.textContent = `${info.info.dayOfWeek[dayofWeek]}`;
      }
      firstDay.textContent = `${info.info.dayOfWeek[dayofWeek]}`;
      dayofWeek++;
      if (dayofWeek > 6) {
        dayofWeek = 0;
        secondDay.textContent = `${info.info.dayOfWeek[dayofWeek]}`;
      }
      secondDay.textContent = `${info.info.dayOfWeek[dayofWeek]}`;
      dayofWeek++;
      if (dayofWeek > 6) {
        dayofWeek = 0;
        thirdDay.textContent = `${info.info.dayOfWeek[dayofWeek]}`;
      }
      thirdDay.textContent = `${info.info.dayOfWeek[dayofWeek]}`;
    ...
    function getCoordinats(posLatitude, posLongitude) {
      ...
      lonMinutes = lon[0];
      lonSeconds = lon[1];
      latitude.textContent = `${info.info.positions.latit} ${latMinutes}°  ${latSeconds}'`;
      longitude.textContent = `${info.info.positions.longit} ${lonMinutes}°  ${lonSeconds}'`;
      buttonSearch.textContent = `${info.info.search.but}`;
      inputCity.placeholder = `${info.info.search.input}`;
    }
